...
When `duowenspec status` is called without `--change` and no changes exist (e.g., during onboarding on a freshly initialized project), the CLI throws a fatal error: `No changes found. Create one with: duowenspec new change <name>`. This breaks the onboarding flow because AI agents may call `duowenspec status` before any change has been created, causing the agent to halt or report failure. Fixes [#714](https://github.com/zxcrf/DuownSpec).

## What Changes

- `duowenspec status` will exit gracefully (code 0) with a friendly message when no changes exist, instead of throwing a fatal error
- `duowenspec status --json` will return a valid JSON object with an empty changes array when no changes exist
...
- `graceful-status-empty`: Graceful handling of `duowenspec status` when no changes exist, covering both text and JSON output modes
