...
The current `openspec init` flow is still optimized for the broad upstream tool matrix and English-first assets. That does not match this fork's intended operator experience: a narrow set of supported assistants, Chinese-first interaction, and the ability to bootstrap a MODO-style admin scaffold without hand-copying another repository.
...
- Restrict `openspec init` tool selection and validation to `claude`, `opencode`, `trae`, `qoder`, `codebuddy`, and `codex`
...
- Add `openspec init --scaffold` to generate a business-empty MODO scaffold based on `modo-frame` and the `b-end-design-pro` modo adapter assets
...
- Ensure `openspec update` preserves the Chinese-first generated assets and scaffold instruction file behavior
