...
- **WHEN** a configured supported tool already contains OpenSpec-managed skill files
- **THEN** refresh the managed content using the current Chinese-first descriptions and guidance
- **AND** preserve any unmanaged content outside the OpenSpec-managed sections when the tool format supports it

#### Scenario: Updating localized command files
- **WHEN** a configured supported tool already contains OpenSpec-managed command or prompt files
- **THEN** refresh the managed content using the current Chinese-first descriptions and guidance
- **AND** preserve command IDs, path conventions, and `/opsx:*` workflow names unchanged
