...
### Requirement: OpenSpec owns the primary workflow
The enterprise workflow SHALL use OpenSpec as the authoritative owner of change stages, artifacts, validation, sync, and archive readiness.
...
- **AND** the contributor SHALL return to the OpenSpec workflow as the source of truth for next-step decisions
...
- **AND** contributors who want the lighter original behavior SHALL be directed to the official OpenSpec distribution rather than a lighter mode in this fork
...
- **WHEN** a contributor uses `openspec config` in this fork
...
- **WHEN** a contributor runs `openspec update` in a project using this fork
...
The enterprise workflow SHALL define where superpowers capabilities may be used within the OpenSpec lifecycle and whether each capability is recommended or required.
...
- **AND** any accepted outcome SHALL be captured through OpenSpec planning artifacts
...
- **AND** task completion SHALL still be determined through the OpenSpec task flow
...
- **AND** OpenSpec SHALL remain the source of truth for whether verification passed
...
- **WHEN** `openspec validate` reports proposal, task, or spec issues
...
- **WHEN** `openspec validate` fails because the current change plan is incomplete, inconsistent, or based on the wrong assumptions
...
- **WHEN** a downstream project uses the published OpenSpec fork and selects the enterprise workflow
...
#### Scenario: Lighter workflow users stay on upstream OpenSpec
- **WHEN** a team wants the lighter original workflow instead of the enterprise-first workflow defined here
- **THEN** they SHALL be able to use the official OpenSpec distribution
