...
- Identify where superpowers adds value without replacing OpenSpec
...
- Publish the result as part of this OpenSpec derivative

**Non-Goals**

- Replacing OpenSpec's artifact model with an external workflow engine
...
- Bundling or vendoring the whole superpowers skillset into OpenSpec if lighter integration is sufficient

## Decisions

### 1. This is a productized workflow extension, not repository-only guidance

The target is a reusable workflow shipped from the current OpenSpec codebase and consumable by other projects after publication.

That means the end state should be more than a README convention. It should be selectable and repeatable through the `openspec` binary.

Expected adoption flow:

- downstream project runs `openspec init`
...
The current OpenSpec product already exists for users who want the lighter original behavior.
...
- teams that want the lighter original workflow should use the official OpenSpec distribution instead of this fork
...
- official OpenSpec distribution: lighter original workflow
...
- `openspec init` establishes the enterprise-first starting state
- `openspec config` manages workflow and delivery choices without reintroducing the lighter upstream flow as a normal sibling mode
- `openspec update` regenerates workflow artifacts in a way that preserves the enterprise-first behavior of this fork
...
### 2. OpenSpec owns stage progression

OpenSpec commands define the official change lifecycle:

- setup: `openspec init`
- exploration: `/opsx:explore`
- plan creation: `/opsx:propose`
- plan validation: `openspec validate`
...
- `brainstorming` stays inside exploration. It helps when inputs are weak or incomplete, but does not replace OpenSpec planning artifacts.
...
The default OpenSpec proposal skeleton is a good base, but the enterprise workflow needs more delivery-oriented planning before implementation starts.
...
This template keeps the current OpenSpec proposal backbone intact while making enterprise release scope and evidence explicit early enough to govern review, verification, and release.
...
- `openspec validate` fails on an existing change: repair the current proposal/design/tasks/spec files directly, then run `openspec validate` again; return to `/opsx:explore` only if the validation issue reveals missing context or a wrong plan
...
- if a later OpenSpec change adds safe regeneration for existing artifacts, this workflow may adopt it, but it should not depend on that future capability today
...
Because this fork is enterprise-first, `openspec config` should not behave like a place where users routinely switch back to the lighter upstream workflow.
...
`openspec update` is the command that turns stored configuration back into generated workflow artifacts.
...
**Risk: sync/archive semantics may conflict with current OpenSpec expectations**
Core OpenSpec already has specific behavior around sync and archive.
...
2. Choose the reusable packaging mechanism inside this fork and wire it into `openspec init`
