...
This fork is not just documenting a preferred way of working. It is doing second-stage development on OpenSpec to ship a reusable enterprise workflow that combines OpenSpec and superpowers without letting them compete for control.
...
- how setup verifies that required external capabilities are actually available
...
Document that OpenSpec remains the source of truth for:
...
### 5. Ship this as a reusable enterprise workflow in the OpenSpec fork

The implementation SHOULD live in the current OpenSpec codebase so other projects can adopt it after this fork is published.

The workflow should not remain repository-only guidance. It should become a reusable workflow offering in this OpenSpec fork, bootstrapped through the `openspec` binary so downstream projects can run `openspec init` and receive the required prompts, skills, and usage guidance.

Document that this fork MUST treat the enterprise workflow as its default generated behavior, rather than trying to preserve the lighter official OpenSpec behavior inside the same published product.

Document that contributors who want the lighter original experience should use the official OpenSpec distribution, while this fork focuses on enterprise-first defaults and enterprise-specific setup behavior.

Document that selecting the enterprise workflow requires preflight capability checks during setup, and that setup MUST block enterprise activation unless the required external capabilities are present or an explicit approved exception is already declared.

Document how `openspec init`, `openspec config`, and `openspec update` each reflect the enterprise-first default of this fork, so setup, reconfiguration, and regeneration do not drift back toward upstream behavior.
...
- update `openspec init` so enterprise-first workflow artifacts are generated by default
- add capability preflight to `openspec init` before enterprise workflow artifacts are written
- update `openspec init` success messaging so the next-step guidance matches the enterprise workflow
- update `openspec config` so workflow and delivery management remain available without presenting the lighter upstream mode as a sibling option
- update `openspec update` so regenerated artifacts preserve enterprise-first behavior
...
Document that the enterprise version of `proposal.md` keeps the default OpenSpec structure but adds delivery-critical sections:
...
Document one complete enterprise `proposal.md` template that combines the preserved OpenSpec base sections with the fixed `Core User Stories`, `Release Coverage`, `Release Evidence Plan`, and `Out of Scope` sections.
...
Document that when `openspec validate` fails on an existing change, the default repair path is to edit the existing planning artifacts and re-run validation, not to assume `/opsx:propose` can regenerate or overwrite those artifacts.

## Capabilities

### New Capabilities

- `superpowers-composed-workflow`: defines the enterprise workflow that combines OpenSpec stages, required superpowers checkpoints, and release-gated completion
...
- `enterprise-capability-preflight`: enterprise setup must confirm required external capabilities before activation
...
- Reusable workflow assets added to this OpenSpec fork so downstream projects can adopt them via `openspec init`
