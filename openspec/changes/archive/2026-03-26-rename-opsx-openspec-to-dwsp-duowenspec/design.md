...
This fork already renamed part of the public surface from `openspec`/`opsx`, but naming is still mixed across binary command entry points, generated command files, templates, and user-facing output.
...
- Ensure core user-facing operation text uses `duowenspec` instead of `openspec`.
...
- Rebranding every historical archived artifact under `openspec/changes/archive`.
...
Alternatives considered:
- Keep `OpenSpec` in user-facing messaging while changing only command keyword: rejected because naming remains inconsistent.
