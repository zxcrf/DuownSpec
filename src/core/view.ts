...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const duowenspecDir = path.join(targetPath, 'duowenspec');
    ...
    console.log(chalk.bold('\nDuowenSpec Dashboard\n'));
    console.log('═'.repeat(60));

    // Get changes and specs data
    const changesData = await this.getChangesData(duowenspecDir);
    const specsData = await this.getSpecsData(duowenspecDir);
  ...
  private async getSpecsData(duowenspecDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(duowenspecDir, 'specs');
