...
- 其内容为 \`modo\`
- 需求来源是 DuowenSpec 产物，而不是 \`.prd\`

## 核心原则

1. **DuowenSpec 是唯一需求来源**
...
2. 读取当前 DuowenSpec 产物，提炼页面目标、用户动作、关键数据和约束
3. 打开 \`duowenspec/b-end/MANIFEST.md\`，了解可用 Biz 组件和角色映射
4. 从以下模式中选择一个最匹配的页面模式：
   ...
   - \`master-tree-table\`
   - \`wizard\`
5. 打开对应模式文档 \`duowenspec/b-end/patterns/<pattern>.md\`
...
开始前先读取：
- \`duowenspec/b-end/MANIFEST.md\`
- 如果需要降级，再读取 \`duowenspec/b-end/references/fallback/<Role>.md\`
...
1. 先确定当前页面需要的角色，例如：
   ...
   - \`PageWrapper\`
   - \`ActionGroup\`
2. 在 \`duowenspec/b-end/MANIFEST.md\` 里查对应映射
...
5. **仍然不可用时再手写**
   - 手写前要参考 \`duowenspec/b-end/references/ANTI_PATTERNS.md\`
...
- \`duowenspec/b-end/references/DESIGN_SPEC.md\`
- \`duowenspec/b-end/references/TOKENS.md\`
- \`duowenspec/b-end/references/THEME_CONFIG.md\`
- \`duowenspec/b-end/references/PITFALLS.md\`
- \`duowenspec/b-end/references/PITFALLS_COMMON.md\`
