...
- 先调用 \`duowenspec-b-end-delivery\`，把当前 DuowenSpec 产物转成统一的页面模式与模板选择
- 把 \`duowenspec/changes/...\` 和 \`duowenspec/specs/...\` 当成需求来源，不要重新回到 \`.prd\`
...
- 开始实现前先调用 \`duowenspec-b-end-delivery\`，确认 pattern、模板和关键组件
- 具体组件解析时调用 \`duowenspec-b-end-components\`，严格以 \`duowenspec/b-end/MANIFEST.md\` 为准
- 输出前必须调用 \`duowenspec-b-end-review\`，确保布局、组件和交互规则保持统一
- 整个过程都以 DuowenSpec 产物为需求来源，不要引回 \`.prd\`
`;
}
...
- 审查时必须调用 \`duowenspec-b-end-review\`
